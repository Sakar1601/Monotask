...
1. **Tinted hover shadows.** Interactive cards (Dashboard stat cards, feature cards) are flat at rest and pick up a soft, primary-tinted shadow only on hover - `shadow-[0_16px_36px_-18px_hsl(var(--primary)/0.55)]` - never a generic black shadow, which would read as an unrelated smudge against a colored card family.
...
### Toasts
Sonner, themed to follow `bg-background` / `text-foreground` / `border-border`, `shadow-lg` on the toast surface. **A real gap worth knowing about:** the `Toaster` component reads its light/dark mode from `next-themes`' `useTheme()`, but this app has no `next-themes` `ThemeProvider` mounted anywhere - the actual app theme is driven entirely through `useSettings().theme` and a manual `.dark` class toggle. In practice this means the toast's own `theme` prop is likely always resolving to its own default rather than following the app's real dark/light state; toasts still render legibly because the CSS classes reference the same `bg-background`/`text-foreground` tokens the rest of the app uses, but this is a latent inconsistency, not a verified-correct integration.
