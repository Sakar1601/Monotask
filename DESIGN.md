...
### Dialogs & Forms
All dialogs share one look from `ui/dialog.tsx` and `ui/alert-dialog.tsx`: a blurred `bg-black/60` scrim, a `rounded-2xl` `bg-card` panel with a tinted deep shadow, and a display-font title (Geist, `text-xl`, tight tracking) with a one-line description beneath. Translucent "glass" panels are no longer used for content dialogs, because forms need a stable, opaque reading surface.
...
Rhythm comes from alternating normal and **inverted** sections (`bg-foreground text-background`), which flips correctly in both themes (a white section on the dark theme, a black one on the light theme). Structure comes from hairline-divided grids (1px `border` gaps inside a rounded container) rather than individual cards. Every figure in the trust band is a fact about how the product is built (for example message scanning is opt-in, Gmail access is read-only), never an invented usage statistic, and integration names are typographic wordmarks rather than reproductions of brand marks. The old GSAP sticky-stack was removed; the page has no scroll-hijacked section. Motion is layered rather than constant: a word-by-word blur-in headline; the hero window tilts up from a 3D perspective as you scroll (scroll-linked, not time-based); three floating status chips drift around it (desktop only); a thin scroll-progress bar sits above the nav; the integrations strip is a slow pausable marquee; problem titles are struck through before their fix brightens; trust numbers count up; feature cells carry a cursor-following spotlight; step dividers draw themselves; and the closing card has a drifting grid with a pinging button halo. Later additions: the theme toggle reveals the new theme as a circle growing from the click point (View Transitions, scoped by a temporary `theme-reveal` class so the app's own transitions are untouched); the hero grid brightens around the cursor on hover-capable devices; and "How it works" is a scroll-driven pinned section on large screens (three viewports tall, `position: sticky`, not a scroll hijack) where the panel beside the steps changes with scroll progress (accounts connect, a suggestion is accepted and another dismissed, synced items land) and each step is also clickable. Small screens and reduced-motion users get plain stacked steps.

Three further additions: a **before/after slider** (`BeforeAfterSlider`, after the problem section) compares one day scattered across an inbox, calendar, chat and to-do app against the same day as one Monotask timeline. It is pointer-draggable, has a keyboard `role="slider"` handle (arrow keys), nudges once on scroll-in, and skips the nudge under reduced motion. Each **feature grid cell** leads with a small looping mini preview (`FeaturePreviews`: week strip, filling habit heatmap, calendar, bars, typing command palette, tag filter, export formats, guest button, theme swap) instead of an icon. Previews loop only while on screen, all share one fixed height so nothing shifts, and reduced motion holds the finished frame. Finally, a monochrome **1200x630 social image** (`public/og-image.png`) backs the `og:image`/`twitter:image` tags with `summary_large_image`; regenerate it if the headline changes.

Two rules learned the hard way: **animated demos must reserve their final size** (an item that appears or disappears must fade in a slot that is always in the layout, never grow or collapse it, or the whole demo visibly jumps each loop); and **the landing root uses `overflow-x-clip`, not `overflow-x-hidden`**, because `hidden` creates a scroll container that silently breaks `position: sticky` for every descendant.

Every animation is gated on `prefers-reduced-motion` (motion components render their end state, CSS keyframes are disabled).
